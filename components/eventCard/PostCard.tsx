...
useEffect(()=>{
  ...
  let leaders: ObjectId[] = [];
  if('team' in post){
     leaders=post.team?.leaders
  }else if('eventImg' in post){
   leaders=post.eventImg?.leaders;
  }

  if(leaders && leaders.length>0){
   const res= leaders.filter((leader)=>leader.toString()===mongoId)
   if(res.length>0)setTeamPostOwner(true);
  }
}
},[])
