    ...
    return (
        <div className="h-fit px-4 pb-4 items-center">
            ...
            <div className="text-cyan-200 mt-5">
                {
                    loggedUser?.interests?.length === 0 || !loggedUser?.interests ? (<><h1>Update your interests to find people sharing same vision as yours</h1>
                        <Link className="mt-2 mx-4 hover:opacity-90 border-2 text-center bg-teal-600 rounded-lg flex" href={`/profile?id=${id}`} >Update Interests</Link>
                    </>
                    ) :
